...
// ── البِنية الثامنة: شاهدٌ داخل النثر ─────────────────────────────────────
// ★ أشيعُ صور الشاهد في كتب الشرح والتفسير والفقه — ولم تكن تُرى البتّة. ★
{
  const page = 'والخلق بمعنى التصوير فى قوله تعالى: (وَإِذْ تَخْلُقُ مِنَ الطِّينِ) '
    + 'أى تصور لا تخترع ومنه قول الشاعر: ولأنت تفرى ما خلقت وبعض القوم يخلق ثم لا يفرى';
  const v = extractVerses(page);
  eq('★ يُقتنص الشاهد من وسط النثر', v.length, 1,
     'لا فاصلَ بين شطريه، ولا سطرَ مستقلّ، ولا قافيةَ متكرّرة — وأداةُ الاقتناص كلُّها على هذه الثلاثة');
  eq('بنصّه كما ورد', v[0].text, 'ولأنت تفرى ما خلقت وبعض القوم يخلق ثم لا يفرى');
  ok('★ ولا يُقسم شطرين بالتخمين', v[0].unsplit === true && v[0].ajz === '',
     'البيتُ مدوَّر («وبعـ/ـض القوم»)، وأيُّ قسمةٍ آليّةٍ تخطئ موضعَ القطع فتحرّف النصّ');

  // ★ ولا يُقبل إلا ما صرّح بالشعر ★
  const prose = 'وقال عطية: مواضع السجود أشد بياضًا يوم القيامة، وهو قول الحسن ومقاتل';
  eq('«قال فلان:» في كتب الشرح نثرٌ مرويّ لا شعر', extractVerses(prose).length, 0,
     'جُرّبت الأداةُ بـ«قال + اسم» فالتقطت هذا النثر — وهو في كل سطرٍ من كتب الشرح');
  eq('و«فساد قول من قال: أن للمرأة…» كذلك',
...
// الاختبارات تحرس المقياس من أن يعود كاذبًا في اتّجاهٍ أو في الآخر.
{
  ...
  // ★ والنسبةُ الموروثةُ تُدقَّق في صفحتها التي جاءت منها ★
  const inherited = { text: 'وكل نعيم لا محالة زائل', poet: 'زفر بن الحرث', poetSource: 'carry' };
  const versePage = 'وكل نعيم لا محالة زائل. ولا اسمَ في هذه الصفحة.';
  eq('فإن وُجد الاسمُ في سابقتها فالنسبةُ مؤيَّدة',
     auditVerse(inherited, versePage, 'وَقَالَ زفر بن الحرث الكلابي:').poet, 'from-prev');
  eq('وإن لم يكن فيها ولا في سابقتها فهي غيرُ مؤيَّدة',
     auditVerse(inherited, versePage, 'صفحةٌ لا اسمَ فيها').poet, 'absent');
  eq('★ وما لم تُجلب سابقتُه لا يُدان ولا يُجمَّل: يُقال لم يُدقَّق ★',
     auditVerse(inherited, versePage).poet, 'carry-unchecked');

  const sumInherit = summarize([
    { verbatim: true, poet: 'from-prev' }, { verbatim: true, poet: 'carry-unchecked' },
  ]);
  eq('فما لم يُدقَّق يُعزل عن المقام', sumInherit.named, 1);
  eq('ويُعلن عددُه', sumInherit.unverifiable, 1);
  eq('والموروثُ المؤيَّد يُعدّ ويُسمّى', sumInherit.fromPrev, 1);
...
// ── البِنية التاسعة: شرحُ الحماسة — متنٌ فوق وشرحٌ تحت ────────────────────
//
// صفحاتٌ حقيقية من «شرح ديوان الحماسة للتبريزي» (٦٩٠٧: ٤٠-٤٢). وفيها ثلاثة
// فخاخٍ لم تكن في الكتب الثمانية قبلها:
//   (١) «ديوان الحماسة» مختاراتُ أبي تمّام من شعر الناس لا ديوانُ رجل،
//       فكان يُخرج قائلًا اسمه «الحماسة» ويُنسب إليه شعرُ معدان وزفر.
//   (٢) الشرحُ في المتن نفسه تحت فاصلٍ «ــ»، وفيه أبياتٌ ليست من القصيدة:
//       «والصواب بدل هذا البيت: (أقدم فيهم دعلجا…)» — بيتُ تصويبٍ للشارح.
//   (٣) القصيدة تنقطع بالشرح في أسفل الصفحة ثم تستأنف في التالية، فالموروثُ
//       إلى الصفحة التالية قائلُ آخرِ المتن لا آخرِ الشرح.
{
  const p40 = '١ - وَقَالَ معدان بن جواس الْكِنْدِيّ\n\n'
    ...
    + 'ــ\n\n١ - معدان بن جواس أحد بني كِنْدَة شَاعِر جاهلي يماني\n';
  const p42 = '(طلقت إِن لم تسألي أَي فَارس ... حليلك إِذْ لَاقَى صداء وخثعما)\n'
    + 'ــ\n١ - طلقت يحْتَمل أَن يكون دُعَاء وحليل الْمَرْأَة زَوجهَا\n'
    + '٢ - وَهَذَا الْبَيْت معيب وَالصَّوَاب بدل هَذَا الْبَيْت\n'
    + '(أقدم فيهم دعلجا وأكره ... إِذا أكْرهُوا فِيهِ الرماح تحمحما)\n';
  const BOOK = 'شرح ديوان الحماسة للتبريزي';

  eq('★ و«الحماسة» اختيارُ أبي تمّام لا شاعرٌ يُنسب إليه ★', poetFromBookName(BOOK), null);

  const a = attributeVerses(p40, extractVerses(p40), { bookName: BOOK });
  eq('ونسبةُ المتن تُقرأ من سطرها المرقَّم', a[0]?.poet, 'معدان بن جواس الكندي');
  eq('وتنتقل بانتقال «وقال فلان»', a[1]?.poet, 'زفر بن الحرث');
  eq('★ والموروثُ إلى الصفحة التالية قائلُ آخرِ المتن لا آخرِ الشرح ★',
     a.carry?.name, 'زفر بن الحرث');

  const b = attributeVerses(p42, extractVerses(p42), { bookName: BOOK, carry: { name: 'عامر بن الطفيل' } });
  eq('فتُستأنف القصيدةُ في الصفحة التالية منسوبةً', b[0]?.poet, 'عامر بن الطفيل');
  const fix = b.find((v) => /أقدم فيهم/.test(v.text));
  ok('★ وبيتُ الشارح لا يرث نسبةَ المتن ★', fix && !fix.poet,
